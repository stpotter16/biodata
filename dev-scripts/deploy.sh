...
SERVICE_LINK="${SERVICE_LINK:-/opt/biodata/server}"
...
echo -e "${BLUE}==> Building biodata package for $TARGET_SYSTEM...${NC}"
...
echo -e "${BLUE}==> Restarting biodata service...${NC}"
ssh -t $TARGET_HOST 'sudo systemctl restart biodata'
